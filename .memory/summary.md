...
- **Next Milestone**: Complete Phase 1 test honesty tasks (`story-c26be0e8`) — two-peer sync, IndexedDB persistence, runtime integration, benchmark rewrite — before closing Phase 1 gate
...
### Phase 1 Test Honesty (NEW — story c26be0e8)
Audit revealed existing tests don't cover WebRTC, IndexedDB, or real engine integration. Four tasks created:
1. `task-2a1e106e` — Two-peer WebRTC sync test (proves transport layer works)
2. `task-86eb4316` — IndexedDB persistence round-trip test (proves persistence works)
3. `task-4df28a4e` — Runtime integration test with real engine (replaces spy-based tests)
4. `task-58dd34a0` — Rewrite or remove fake benchmark (depends on #1)

These must pass before Phase 1 can be considered gate-ready.
